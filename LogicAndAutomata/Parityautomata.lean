...
| i =>
  ...
  -- let l2 := ∑ m ∈ Finset.range i, (subset_wb_f_pair w ρ_w_pareven m).2 + 1
  ...
  have notloopinletterstate : ∃ n>0, ¬(∃q, ρ_w (l2+n) = (q, Sum.inl (w l2))) := by
    ...
    have infocceqone : InfOcc ((A.StutterClosed).parityMap ∘ ρ_w) = {1} := by
      ...
      have infoccnonemp : (InfOcc ((A.StutterClosed).parityMap ∘ ρ_w)).Nonempty := by
        rw [infOcc_comp_of_Finite (A.StutterClosed).FinState ρ_w]
    ...
    have ssupodd : Odd (sSup ((A.StutterClosed).parityMap '' InfOcc ρ_w)) := by
      rw [ssupone]
      exact odd_one
...
| i =>
  if hf : f i = 0 then
    ...
    have this : ∃ n,  ∃ ρ : Stream' A.State, ∃ n_ge : n ≥1,
...
lemma subset_stutequiv_w_w' {A : NPA Alph} {w : Stream' Alph} {f : Stream' ℕ}
        {ρ_w : Stream' (A.StutterClosed).State} (ρ_w_run : A.StutterClosed.InfRun w ρ_w)
        (ρ_w_pareven : Even (sSup ((A.StutterClosed).parityMap '' InfOcc ρ_w)))
        (hf : f = subset_f w ρ_w_pareven) :
        StutterEquivalent w (functiononword (subset_wb w ρ_w_pareven)
          (subset_f' ρ_w_run ρ_w_pareven hf)) := by
...
lemma subset_rhow'_run {A : NPA Alph} {w wb: Stream' Alph} {f f' : Stream' ℕ}
  ...
  · unfold subset_rhow'
    ...
    split
    case isTrue hf0zero =>
      simp only [Nat.zero_sub_one, zero_tsub]

      have := (subset_f'_rhow'_pair._proof_4 ρ_w_pareven (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf 0))
        (subset_f'_rhow'_pair._proof_2 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven
          (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf 0))
          (Eq.mpr_prop (Eq.refl (f (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf 0)) = 0))
            hf0zero))).choose_spec
      simp only at this
      obtain ⟨w , ⟨hfinrunstart, right⟩⟩ := this
      clear right
      clear w

      have mzero : (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf 0)) = 0 := by
        rw [Nat.find_eq_zero]
        unfold n_lt_sumk
        simp
      have sumzero : (∑ m : Fin ((Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf 0))), (f ↑m + 1)) = 0 := by

        sorry
      --- ??? Wat gebeurt hier toch
      -- simp at hfinrunstart
      -- rw [sumzero] at hfinrunstart



      -- simp only [hf0zero] at hfinrunstart


      sorry
    case isFalse hf0nonzero =>
      simp
      unfold NA.init at ρ_w_init
      unfold NPA.StutterClosed at ρ_w_init
      simp only [Set.mem_setOf_eq] at ρ_w_init
      obtain ⟨s, ⟨hsinit, hs⟩⟩ := ρ_w_init
      rw [Prod.eq_iff_fst_eq_snd_eq] at hs
      simp at hs
      rw [← hs.1]
      exact hsinit
  ...
  · intro k
    ...
    unfold subset_f'_rhow'_pair
    simp only
    let i_b := Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf (k + 1))
    have hib : i_b = Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf (k + 1)) := rfl
    -- let i_b := Nat.find (kexists k (subset_f' ρ_w_run ρ_w_pareven hf))
    -- have hib : i_b = Nat.find (kexists k (subset_f' ρ_w_run ρ_w_pareven hf)) := by trivial
    if hfzero: f i_b = 0 then
      rw [hib] at hfzero
      -- rw [← Nat.add_sub_cancel k 1] at hfzero
      split
      case isTrue hzero =>
        simp only
        have spec1 := (subset_f'_rhow'_pair._proof_4 ρ_w_pareven (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf (k + 1)))
  (subset_f'_rhow'_pair._proof_2 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven
    (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf (k + 1)))
    (Eq.mpr_prop (Eq.refl (f (Nat.find (subset_rhow'._proof_1 ⟨ρ_w_init, ρ_w_next⟩ ρ_w_pareven hf (k + 1))) = 0))
      hfzero))).choose_spec
        obtain ⟨n_ge, ⟨left, right⟩⟩ := spec1
        clear right
        clear n_ge

        unfold NA.FinRunStart at left


        sorry
      case isFalse hnonzero =>
        -- Geen ideeeeeeeee grrrrr
        -- exact hfzero hnonzero
        sorry
    -- Vragen waarom niet werkt
      -- split
      -- .
      -- .
      -- conv =>
      ...
      -- -- hier vragen hoe je niet alles kan simpen
      -- conv =>
      --   rhs
      --   lhs
      ...
      --   sorry
      -- simp only [gt_iff_lt, ge_iff_le, exists_and_left, exists_and_right, ↓reduceDIte,
      --   add_tsub_cancel_right, Nat.add_one_sub_one, zero_add]

      -- rw [add_tsub_cancel_right] at hfzero
...
lemma sumpointseqnumstate {A : NPA Alph} {w : Stream' Alph} {ρ_w : Stream' (A.StutterClosed).State}
  ...
  case zero =>
    ...
    simp only [gt_iff_lt, Finset.univ_eq_empty, Finset.sum_empty, zero_add,
      Nat.le_find_iff, Nat.lt_one_iff, not_and, not_not, forall_eq, lt_self_iff_false,
        IsEmpty.forall_iff, Nat.sub_add_cancel, Nat.lt_find_iff] at hi_b
    have ρ_w_next_spec : ρ_w (subset_f w ρ_w_pareven 0 + 1) ∈
        (A.StutterClosed).next (ρ_w (subset_f w ρ_w_pareven 0)) (w (subset_f w ρ_w_pareven 0)) :=
    ...
    have natfind_eq_iff :=
        (Nat.find_eq_iff (m:= Nat.find (subset_wb_f_pair._proof_1 w ρ_w_pareven 0))
        ((subset_wb_f_pair._proof_1 w ρ_w_pareven 0)))

    simp only [gt_iff_lt, Finset.univ_eq_empty, Finset.sum_empty, zero_add, not_exists,
      Nat.lt_find_iff, nonpos_iff_eq_zero, not_and, not_forall, Decidable.not_not, forall_eq,
      lt_self_iff_false, IsEmpty.forall_iff, true_and, true_iff] at natfind_eq_iff
    simp only [zero_add, Finset.range_one, gt_iff_lt, not_exists, Nat.le_find_iff, Nat.lt_one_iff,
      not_and, not_forall, Decidable.not_not, forall_eq, lt_self_iff_false, add_zero,
      IsEmpty.forall_iff, Nat.sub_add_cancel, Finset.sum_singleton, Finset.univ_eq_empty,
      Finset.sum_empty]
    use q
    obtain ⟨notloopstate, _⟩ := natfind_eq_iff
  ...
  case succ n =>
    ...
    have nat_find_lt: (((Nat.find (subset_wb_f_pair._proof_1 w ρ_w_pareven (n+1))) - 1)) <
    ...
    have ρ_w_next_spec : ρ_w ((∑ x ∈ Finset.range (n + 2), (f x + 1))) ∈
        (A.StutterClosed).next (ρ_w ((∑ x ∈ Finset.range (n + 2), (f x + 1)) -1))
    ...
    have finsetrangesucc_eqfinsum : (∑ m : Fin (n+1), ((subset_wb_f_pair w ρ_w_pareven ↑m).2 + 1)
        + (Nat.find (subset_wb_f_pair._proof_1 w ρ_w_pareven (n+1)) - 1)) =
        (∑ x ∈ Finset.range (n + 2), (f x + 1) - 1) := by
      nth_rewrite 1 [Fin.sum_univ_eq_sum_range
          (fun m ↦ ((subset_wb_f_pair w ρ_w_pareven m).2 + 1)) (n+1)]
      nth_rewrite 2 [Finset.sum_range_succ]
      rw [hf]
      unfold subset_f
      simp only
    ...
    rw [← finsetrangesucc_eqfinsum] at ρ_w_next_spec
    rw [hq] at ρ_w_next_spec
    simp at ρ_w_next_spec
    -- simp at k2_spec
    ...
    rw [hk2] at k2_spec
    rw [← Nat.add_sub_assoc (k:=1) (by rw [← hk2]; omega)] at finsetrangesucc_eqfinsum
    rw [← Nat.add_left_inj (n:=1)] at finsetrangesucc_eqfinsum
    rw [Nat.sub_add_cancel (by rw [← hk2]; omega)] at finsetrangesucc_eqfinsum
    rw [Nat.sub_add_cancel (by rw [Finset.sum_range_succ]; omega)] at finsetrangesucc_eqfinsum
    rw [finsetrangesucc_eqfinsum] at k2_spec
...
lemma subset_rhow'_pareven {A : NPA Alph} {w : Stream' Alph} {ρ_w : Stream' (A.StutterClosed).State}
    ...
    have hslininfoc : (sl + 2) ∈ (InfOcc ((A.StutterClosed.parityMap) ∘ ρ_w)) := by
      ...
      case isTrue hfiwzero =>
        ...
        · let h1 := (subset_f'_rhow'_pair._proof_2 ρ_w_run ρ_w_pareven iw'_b
          ...
          let h2 := (subset_f'_rhow'_pair._proof_4 ρ_w_pareven iw'_b h1)
          let ρ_fin := h2.choose
          ...
          obtain ⟨ n_ge,finrunstart, max, e⟩ := ρ_fin_spec

          rw! [Fin.sum_univ_eq_sum_range (fun n ↦ f n + 1) iw'_b] at e
          -- simp only [gt_iff_lt, ge_iff_le] at e
          ...
          -- simp [sumpointseqnu]
          -- simp [max]
          rw! [Fin.sum_univ_eq_sum_range (fun n ↦ f n + 1) iw'_b] at max
          rw! [← Finset.sum_range_succ] at max
          rw! [max]
          rw [Sum.elim_inr]
          simp only
          ...
          have same : h1.choose_spec = h2 := by sorry

          let spec1 := h1.choose_spec
          let spec2 := spec1.choose_spec
          unfold spec1 at spec2
        ...
        -- let n :=
        -- have spec1 := (subset_f'_rhow'_pair._proof_1 ρ_w_run ρ_w_pareven iw'_b (Eq.mpr_prop (Eq.refl (f iw'_b = 0)) hfiwzero)).choose_spec
        ...
        -- have spec1 := (subset_f'_rhow'_pair._proof_1 ρ_w_run ρ_w_pareven iw'_b).choose_spec
        -- have spec2 := spec1.choose_spec
  ...
  have ssupge: (sSup (A.parityMap '' (InfOcc (subset_rhow' ρ_w_run ρ_w_pareven hf)))) + 2 ≥
    ...
    obtain ⟨x, ⟨hxinf, hxsup⟩⟩ := srinrange
...
noncomputable def supset_rhow {A : NPA Alph} (ρ_wb : Stream' (A.StutterClosed).State)
  ...
  if k = 0 then
    ...
    if fkb_zero : f k_b = 0 then
      ...
      -- have dec: Decidable (((ρ_wb (k_b + 1)).1, Sum.inl (w k)) ∈ ((A.StutterClosed).next (supset_rhow ρ_wb ρ_w' w f f' (k - 1)) (w k))) := by sorry
...
lemma rhowb_numstate {A : NPA Alph} (f' : Stream' ℕ) (ρ_w' : Stream' (A.State)) (k : ℕ) :
  ...
  case succ n =>
    ...
    if h1 : (f' n) = 0 then
      ...
      simp only [↓reduceIte, h1, Sum.inr.injEq, Subtype.mk.injEq, exists_prop, exists_eq_right']
      refine @ssupinrange Alph A ((ρ_w' '' {l | ∑ m ∈ Finset.range n, (f' m + 1) < l ∧ l ≤ ∑ m ∈ Finset.range n, (f' m + 1) + f' n + 1})) ?_ ?_
...
-- Full theorem
theorem NA.StutterClosurerecognizesStutterClosure (A : NPA Alph) :
  ...
  · intro hwinlang
    ...
    · exact subset_stutequiv_w_w' ρ_w_run ρ_w_pareven rfl
  · intro hwinlang
