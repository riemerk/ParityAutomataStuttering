...
Syppose we have $w\in \LL(\A^s)$ if we want to show that $w\in(\LL(\A))^s$ we need to find a word $w'\in \LL(\A)$ such that $w\sim_sw'$. Or more specifically we need to find a $w_b\in \Sigma^\omega$ and $f, f':\omega\to\omega^+$ such that $w=w_b[f]$ and $w'=w_b[f']$. Firstly I will define $w_b$ and $f$. We take as input $\rho_w$ and $w$
\begin{definitiont}\label{def:subsetwbf} To define $w_b$ and $f$ take $i\in \omega$. Let $l := \sum_{m=0}^{i-1}f(m)$. Since $\rho_w$ is an accepting run we know it cannot loop infinitely in a state of the form $(q,a)\in Q_\A\times\Sigma$ since $\Omega_{\A^s}(q,a)=1$ and not even. So therefore there exists a smallest $n>0$ such that $\rho_w(l+n)\in Q_\A\times\Omega_\A(Q_\A)$. Now set $f(i):=n$ and $w_b(i) := w(l)$
\end{definitiont}
We need to prove that this $w_b$ and $f$ are indeed correct.
\begin{claim}\label{claim:subsetwwb[f]}
 Let $w_b$ and $f$ from Definition \ref{def:subsetwbf} then $w=w_b[f]$.
\end{claim}
\begin{proof}
...
\end{proof}
Our second step is to define a $w'$ such that $w\sim_s w'$ and show that $w'\in\LL(\A)$. For $w'$ take $w_b[f']$ with $f'$ defined in the next definition. To show that $w'\in\LL(\A)$ we will define $\rho_{w'}$.
\begin{definitiont}\label{def:subsetrhow'_f'}
 To define $f'$ and $\rho_{w'}$. Take $i\in \omega$. Firstly $(q, p) = \rho_w(0)$ then set $\rho_{w'}(0) = q$. Then take $i\in\omega$ and distinguish cases:
\begin{description}
...
\begin{description}
 ...
 \item[Case $f(i_b)=n$] Now we see that $q=\rho_w\left(\sum_{m=0}^{i_b}f(m)\right)$ dus $\Omega_{\A^s}(\rho_w(\sum_{m=0}^{i_b}f(m)))-2=\Omega_\A(q)$.
\end{description}
