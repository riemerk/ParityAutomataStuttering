\begin{definition}\label{def:APAtoNPA}
 Given an Alternating Parity Automaton (APA) $\mathbb{A}=(\Sigma, Q, q_0,\delta, \Omega)$ define the Nondeterministic Parity Automata (NPA) $\mathbb{A}'=(\Sigma, Q', q_0',\delta', \Omega')$ as follows:
 \begin{itemize}
  ...
  \item Define the set of initial states as $q_0' \subseteq Q' = \begin{cases}
               ...
               \end{cases}$
  \item Define the parity map $\Omega' : Q'\to \mathbb{N}$ as:
  \[
  ...
  \]
  \end{itemize}
  First some preliminary notationally abbreviations. Define $Z_{\min} : Q\times \Sigma \to \PP(Q)$ as the minimal subset such that $Z_{\min}(q,a)\models\delta(q,a)$. Define $\pi : \PP(\omega)\times \omega\to \omega$. For $\pi(X, p)$ distinguish two cases:

  if $\text{par} := \{\max(p, x)\mid x\in X\}$ contains an odd parity set $\pi(X, p)$ to the maximum odd parity in this set.

  Else set $\pi(X, p)$ to the minimum (even) parity in $\{\max(p, x)\mid x\in X\}$.

  For the more symbolic reader:
  \[
   \pi(X, q) :=\begin{cases}
                \max\{p\in \text{par} \mid p \text{ odd }\} &\text{if par contains odd parity}\\
                 \min(\text{par}) &\text{else}
               \end{cases}
  \]

  To define the transition function $\delta' : Q'\times \Sigma \to \mathcal{P}(Q')$ take a $(X,p)\in Q'$ and a $a\in \Sigma$. Now distinguish two cases:\\
  \begin{itemize}
  ...
  \begin{align*}
   ...
   &\phantom{\cup\Biggl\{}\left\{q|(q, p_q)\in Y\right\}=Z \land p_q=\begin{cases}0&\text {if }\Omega(q) \text{ odd } \\ \Omega(q)&\text{else}\end{cases} \text{ for every } (q, p_q)\in Y
   \Biggr\}
  \end{align*}
  \item[\textbf{Case non-final:}] There is a $(q, p)\in X$  where $p$ is odd.\\
  That means that not all paths have visited a maximum even parity. \\
  Now set $\delta'$ to'\\

    \begin{align*}
    \delta'((X, p), a) &= \Biggl\{ (Y, 0) \mid Z:=\{q|(q, p_q)\in Y\}\subseteq Q, \text{ such that }
    ...
    &\cup \Biggl\{ (Y, 1) | \mid Z:=\{q|(q, p_q)\in Y\}\subseteq Q, \text{ such that } Z\models \delta(q', a)\text{ for every }(q', p_{q'})\in X\\
    &\phantom{\cup\Biggl\{}\land p_q=\pi\left(\{p_{q'}\mid (q', p_{q'}\in X\land q\in Z_{\min} (q', a)\},\begin{cases}0&\text {if }\Omega(q) \text{ odd } \\
      \Omega(q)&\text{else}\end{cases}\right)\\
      &\phantom{\cup\Biggl\{}\text{ for every } (q, p_q)\in Y\Biggr\}
    \end{align*}
\end{itemize}
\end{definition}
\begin{theorem}\label{thm:APAtoNPA}
The NPA from definition \ref{def:APAtoNPA} indeed recognizes the same language as the APA in other words:
\[\LL(\A)=\LL(\A')\]
...
Take $w\in \LL(\A)$, then we know that there exists an accepting run $\rho_\A=(V, E)$ on $w$ in $\A$. Without loss of generality assume $\rho_\A$ is minimal.\\
...
\textbf{Definition}:$\rho_{\A'}=q'_0q'_1q'_2\dots$. \\
First we define for $i=0$
\[q'_0 = \begin{cases}
...
\begin{itemize}
 \item[\textbf{Final}] If $q'_{i-1}=(X, p)$ was final (i.e. $p_q$ is even for every $(q,p_q)\in X$):
 ...
 \begin{itemize}
  ...
  \[
   q'_i = \left(\left\{(q, p_q)\mid(q, i)\in V, \text{ and } p_q=\begin{cases}
            0 & \text{ if } \Omega(q) > \max(\inf_\Omega(\rho))\\
            \Omega(q) & \text{ else }
            \end{cases}\right\}, 1\right)
  \]

 \end{itemize}
\item[\textbf{Non-final}]If $q'_{i-1}=(X, p)$ was non-final (i.e. $p_q$ is odd for a $q\in X$). Distinguish two cases:
\begin{itemize}
  \item[C)] If \(\Omega(q)\leq \max(\inf_\Omega(\rho, (q,i))) \) for all $(q, i)\in V$ then set:
  \[
  q'_i = (\{(q, p_q)|(q, i)\in V, p_q=\pi(\{p_{q'}|(q', p_{q'})\in X\land q\in E_{(q', i-a)}\}, \Omega(q)), 0)
  \]
  \item[D)] Else: If \(\Omega(q)> \max(\inf_\Omega(\rho, (q,i))) \) for a $(q,i)\in V$ then set:
  \begin{multline*}
   q'_i = \Biggl(\Biggl\{(q, p_q)|(q, i)\in V, \text{ where }
            p_q=\pi(\{p_{q'} |(q', p_{q'})\in X\land q\in E_{(q', i-1)}\}, \\
            \begin{cases} \Omega(q)&\text{if } \Omega(q)\leq \max(\inf_\Omega(\rho, (q,i)))\\0&\text{else} \end{cases}\Biggr\}, 1\Biggr)
    \end{multline*}
 \end{itemize}
\end{itemize}

\Claim (1): $\rho_{\A'}$ is a well-defined run on $w$ in $\A'$

\Proof: I will show that for every $q'_i$ indeed $q'_i\stackrel{w_i}{\longrightarrow}q'_{i+1}$. Take $q'_i = (X_{i}, p_i)$  and we disinguish the same cases as in the definition of $\rho_{\A'}$. We will use $\{q\mid (q,p_q)\in X_i\}=\{q\mid (q,i)\in V\}$
\begin{itemize}
 \item[A and B)] Take $Z=\{q \mid (q,i+1)\in V\}$. Now for every $(q, i)\in X_{i}$ we know from the fact that $\rho_\A$ is a run (with R3) that $E_{(q,i)}\subset Z$ and $E_{(q,i)}\models\delta(q, a_{i})$ so also $Z\models\delta(q, a_{i})$. We also see that the definition of $p_q$ corresponds to the definition of $p_q$ in $\delta'$. So that means that $q'_{i+1}\in \delta'(q'_{i, a_i})$
...
\end{itemize}
\Claim (2): $\rho_{\A'}$ is accepting, i.e. $\max(\inf_\Omega(\rho_{\A'}))$ is even.

\Proof: Now if $\rho_\A$ is an accpeting run on $w$ in $\A$ that means that on any infinite path the maximum parity in the inf set is even. That means that we can identify a timestep $i$ such that after this time every infinite path only visits states with an even parity. Now we will look at $\rho_{\A'}$ again. Now I claim that the maximum priority on $\rho_{\A'}$ is even. We see that after this timestep there is no time $j$ when there occurs a state $q$ with $\Omega(q)>\max(\inf_\Omega(\rho, (q,j)))$ since we have assumed that on every infinite path the maximum parity in the inf set is the maximum parity. So that means that the parity 3 cannot occur in $\rho_{\A'}$ after i. Now we also have that every infinite path infinitely often visits its (even) maximum parity. When all paths have done this we are in a final state with parity 2 and we reset. Since every infinite path infinitely often visits this maximum parity we see that $\rho_{\A'}$ infinitely often visits a final state so that means that $\max(\inf_{\Omega_{\A'}}(\rho_{\A'}))$ is even.
...
From the fact that $\rho$ is an accepting run on $w$ we now know that $w\in \A'$ which proves the inclusion. \qed\\
...
\Claim (4) $\rho_\A$ is accepting on $w$ in $\A$

\Proof: Since we know that $\rho_{\A'}$ is accepting we know that the $\max(\inf_{\Omega_{\A'}}(\rho_{\A'}))$ is even. Suppose $\rho_\A$ is not accepting. That means there is an infinite path $u$ where $\max(\inf_{\Omega_\A}(u))$ is odd. Now that means that there is a $i$ such that for every $u[j]$ with $j>i$ we have $u[j]\leq \max(\inf_{\Omega_\A}(u))$ (and $u[i]=\max(\inf_{\Omega_\A}(u))$). Now look at the the run $\rho_{\A'}$ after this $i$ we see that for every $q_j\in u$ we never set $p_{q_j}$ to an even parity since it starts at the odd parity $\max(\inf_{\Omega_\A}(u))$ and on $u$ it never encounters a bigger even parity. That means that from this point $\Omega_{\A'}(q_j)=q$ for $q_j\in \rho_{\A'}$ so therefore $\max(\inf_{\Omega_{\A'}}(\rho_{\A'}))=1$ and that contradicts to the fact that $\rho_{\A'}$ is accepting. Therefore every infinite path $u$ in $\rho_A$ should be accepting so that means $\rho_\A$ is also accepting so that means that $w\in \LL(\A)$ and that proves that indeed
\[
 \LL(\A)=\LL(\A'),
\]
as desired.
