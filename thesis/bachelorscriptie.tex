...
Let $\mathbb{G}$ a parity formula over $\Psf$. Define the \textbf{language} of a parity formula as:
 ...
 \]
\end{definition}
\begin{definition}\label{def:posstrattree}
 Define the \textbf{positional strategy tree} as a variant of the strategy tree for positional strategies. If $f$ is a surviving positional strategy for player $\sigma$ in $\mathcal{B}@a$ we can represent $f$ as the tree: $\mathbb{T}_a^{f_{pos}}:=(\PP(B), \stackrel{\rightarrow}{E}_f, a)$ where we have (even opschrijven dat je de partial matches gewoon schrijft met hun laatste node ipv hele partial match want positional)
 \[
  \stackrel{\rightarrow}{E_f}(p) := \begin{cases}
                                     \{(p, p')\mid p'\in E[p]\} &\text{if } p\in B_{\overline{\sigma}}\\
                                     \{f(p)\} & \text{else}
                                    \end{cases}
 \] Since the strategy of $f$ only depends on the last node in a partial match.
\end{definition}
...
\end{theorem}
\begin{proof}
 Let $\mathbb{G}=(V, E, L, \Omega, v_I)$ a strongly guarded and simple parity formula. We will define $\A=(\Sigma, Q, q_0, \delta, \Omega')$ as follows:
 \begin{itemize}
...
\textit{Proof}: First $\Mod_\Psf(\mathbb{G}) \subseteq \LL(\A)$. Suppose $w=a_0a_1a_2\dots \in \Mod_\Psf(\mathbb{G})$ then we know that $(v_I, 0)$ is a winning position for $\exists$. This means there is a winning strategy $f$ for $\exists$ and since $\mathbb{G}$ is a parity game we even know that there exists a positional winning strategy $f$ for $\exists$ in $\mathcal{B}@(v_I,0)$. Now take the positional strategy tree $\mathbb{T}_{(v_I,0)}^f$, we are going to construct a succesfull run in $\A$ based on this positional strategy tree. Recall that a run in $\A$ is a DAG $\rho=(V_\rho,E_\rho)$ where $\rho$ satisfies four conditions R1-R4. Define the nodes
\[
 V_\rho = \{(v_I, 0)\}\cup\bigcup_{i\geq 1}\left\{(v, i)\mid \exists v'\in V, L(v')=\deo, \{v\}=E[v'], (v', i-1)\in \mathbb{T}_{(v_I,0)}^f \right\}
\]
and the edges
\[
 E_\rho = \bigcup_{i\geq 0}\left\{((v, i), (v', i+1))\in V\times V\mid (v', i+1)\in\mathbb{T}_{(v,i)}^f \right\}
\]
Now we have to check that this is indeed a run. Satisfaction of R1-R3 is an easy verification since $\mathbb{T}_{(v_I,0)}^f$ is already a tree and. For the satisfaction of R4 we need to dive in the definition of $\delta$. Take $(q,l)\in V$.\\
\textbf{Claim:} $E_q=\{q'\mid ((q, l), (q', l+1))\in E\} \models \delta(q, a_l)$\\
\textit{Proof}: I will prove this via induction on the complexity on $\delta(q,a_l)$. Since $\mathbb{G}$ is guarded there are no loops so we cannot loop infinitely over $\delta$ and get a fixpoint. \textcolor{red}{IH goed formuleren, iets met $\delta$ en $E[v]$, en hier heb je dus $v\leq q$ eigenlijk in de parityformuleboom}: Induction hypo: for every $v\in \mathbb{T}^f_{(q, l)}$ we have $E_v\models (v, a_l)$ The base cases are: $L(v)=\deo$ or $L(v)\in\mathtt{At}(\Psf)$. Now if $L(v
