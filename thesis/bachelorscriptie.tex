...
\chapter{Form $\mutl$ formulas to automata}
As described in the introduction the first goal of this thesis is to translate a $\mutl$ formula to a $\omega$-automaton. In this chapter we will give this translation, more specificaly to a Non deterministic Parity Automaton. We will start the chapter with a constraint on $\mutl$ formulas that is neccesary for this specific translation: modally simpleness. In section 3.2 we will firs translate $\mutl$ formulas to parity formulas which we translate to APA's in Section 3.3. In Section 3.4 we give a translation from alternating automata to Nondeterministic automata and in Sectoin 3.5 we finally conclude by combining all the theorems in one translation from $\mutl$ to a nondeterministic parity automata.
...
\section{Notational Conventions}\label{apdx:notation}
\input{notation.tex}
